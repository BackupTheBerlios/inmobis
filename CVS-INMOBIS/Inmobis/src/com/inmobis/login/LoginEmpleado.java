...
public class LoginEmpleado extends Login{
	private static final Logger milog = Logger.getLogger(LoginEmpleado.class);

	@Override
	public ActionMessages HazLogin(Object objLogin) {

		if (milog.isInfoEnabled()){
			milog.info("Estoy en HazLogin de LoginEmpleado");
		}
		...
		{
			errors.add("login", new ActionMessage("error.bbdd.conexion"));
			if (milog.isInfoEnabled()){
				milog.info("LoginEmpleado Excepcion en la base de datos:"+E.getMessage());
			}
			return errors;
		}
		if (milog.isInfoEnabled()){
